...
- 轻量级锁依赖 CAS 操作 Mark Word来试图获取锁，如果重 试成功，就使用普通的轻量级锁;否则，进一步升级为重量级锁。
...
作者说---我个人认为，能够基础性地理解这些概念和机制，其实对于大多数并发编程已经足够了，毕竟大部分工程师未必会进行更底层、更基础的研发，很多时候解决的是知道与否，**真正的提高还要靠实践踩坑**。
 ...
 1. 首先，synchronized 的行为是 JVM runtime 的一部分，所以我们需要先找到 Runtime 相关的功能实现。通过在代码中查询类似`“monitor_enter”`或`“Monitor Enter”`，很直观 的就可以定位到:
```c++
...
- slow_enter 则是绕过偏斜锁，直接进入轻量 级锁获取逻辑。


类似 `fast_enter` 这种实现，解释器或者动态编译器，都是拷贝`synchronizer.cpp`这段基础逻辑，所以如果我们修改这部分逻辑，要保证一致性。
